...
raw_data_path_flow <- "~/projects/Group25_Project/_raw/flowjo_info"

data_path <- "~/projects/Group25_Project/data"
data_path_flow <- "~/projects/Group25_Project/data"
...
outfile_path_flow <- file.path(data_path_flow, outfile_name_flow)
...
data_flow <- map(file_paths_flow, 
                 ~read_excel(.x, col_types = 'text') |> 
                  ~rename(.x, experiment = 1) |> 
                   ~filter(row_number() <= n() - 2) |>  # remove access columns
                   ~mutate_all(str_remove(.x, " %"))) |> 
                   mutate_all(~as.numeric(str_replace(.x, ",", ".")))
                   
```

```{r}
data_test <- map(file_paths_flow, 
             ~read_excel(.x, col_types = 'text')) |>
  map(~colnames(.x))
```

```{r}
data_ <- map(file_paths_flow, 
